...
function EllieAIView({ theme, ellieTheme, outfit, setEllieOpen, askEllie, generateExecutiveBrief, briefLoading, executiveBriefs, selectedBriefId, setSelectedBriefId, downloadExecutiveBrief,executiveBrief,showExecutiveBrief, }) {
  return (
    <motion.section
      key="ellie"
      initial={{ opacity: 0, y: 20 }}
...
{showExecutiveBrief && executiveBrief && (
  <GlassCard className="mt-6 p-7" theme={theme}>
    <div>
      <p
        className="text-xs font-black uppercase tracking-widest"
        style={{ color: theme.dark }}
      >
        Ellie AI Executive Intelligence
      </p>
    ...
    <div className="mt-6 whitespace-pre-wrap text-sm leading-7 text-[#53635a]">
      {executiveBrief}
    </div>
  </GlassCard>
)}
...
export default function PulseIntelligence() {
  ...
  const [executiveBriefs, setExecutiveBriefs] = useState(() => {
    try {
      return JSON.parse(
        localStorage.getItem("ellieExecutiveBriefs") || "[]"
      );
    } catch {
      return [];
    }
...
const generateEllieAnswer = async (prompt) => {
  ...
  if (!responseText) {
    throw new Error(
      `Ellie AI returned an empty response. HTTP status: ${response.status}`
    );
  }
  ...
  try {
    ...
    throw new Error(
      `Ellie AI returned invalid JSON. HTTP status: ${response.status}. Response: ${responseText.slice(0, 200)}`
    );
  }
  ...
  if (!response.ok) {
    throw new Error(
      result?.message ||
        `Ellie AI request failed with status ${response.status}.`
    );
  }

  if (!result.answer) {
    throw new Error(
      "Ellie AI responded successfully but did not return an answer."
    );
  }
...
const downloadExecutiveBrief = (brief) => {
  const content = `
PULSE INTELLIGENCE
ELLIE AI EXECUTIVE SUMMARY
...
const generateExecutiveBrief = async () => {
  if (!historicalComparisonContext) {
    alert(
      "Please select a comparison period before generating an executive summary."
    );
    return;
  }
  ...
  try {
    const answer = await generateEllieAnswer(
      `Generate a professional one-page executive summary.
...
Use only the supplied Pulse analytics context.
Keep the summary concise and appropriate for executive leadership.`
    );
    ...
    setExecutiveBriefs(updatedBriefs);
    ...
    alert(
      error instanceof Error
        ? `Executive summary error: ${error.message}`
        : `Executive summary error: ${String(error)}`
    );
      ...
      case "Ellie AI":
        return (
          <EllieAIView
            ...
            executiveBriefs={executiveBriefs}
            selectedBriefId={selectedBriefId}
            setSelectedBriefId={setSelectedBriefId}
            downloadExecutiveBrief={downloadExecutiveBrief}
